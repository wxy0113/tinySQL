...
public class Join {
    ...
    public static ArrayList<Tuple> onePassJoin(SchemaManager schema_manager, MainMemory memory, String tableOne, String tableTwo){
    ...
    public static ArrayList<Tuple> nestedLoopJoin(SchemaManager schema_manager, MainMemory memory, String tableOne, String tableTwo){
        ArrayList<Tuple> tuples = new ArrayList<Tuple>();
        Relation TableOne = schema_manager.getRelation(tableOne);
        Relation TableTwo = schema_manager.getRelation(tableTwo);
        int sizeOne = TableOne.getNumOfBlocks();
    ...
    @SuppressWarnings("Duplicates")
    public static Schema combineSchema(SchemaManager schemaManager, String tableOne, String tableTwo){
    ...
    public static Tuple combineTuple( Relation relation, Tuple smalltuple, Tuple largetuple){
        ...
        for(int i = 0; i<smallSize+largeSize; i++){
            if(i<smallSize){
                ...
                if(ExpressionTree.validInteger(temp)){
        ...
        for(int i=0; i<largeRelation.getNumOfBlocks(); i++){
            ...
            for(Tuple largeTuple:largeRelationTuples){
                for(Tuple smallTuple:smallRelationTuples){
                    ...
                    if(ExpressionTree.validInteger(result1) && ExpressionTree.validInteger(result2)){
    ...
    public static ArrayList<Tuple> twoPassNaturalJoin(SchemaManager schemaManager, MainMemory memory, String tableOne, String tableTwo, String fieldName){
        ...
        PhysicalQuery.twoPassHelper(t1, memory, fieldName);
        PhysicalQuery.twoPassHelper(t2, memory, fieldName);
        ...
        if (!schemaManager.relationExists(name)) {
            crossRelation = schemaManager.createRelation(name, schema);
        } else {
            crossRelation = schemaManager.getRelation(name);
        }
    ...
    public static int compareFields(Field a, Field b){
    ...
    public static int compareTuples(Tuple a, Tuple b, String fieldName){
